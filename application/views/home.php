		<!-- START PAGE WRAPPER -->
		<div class="playbook">
		<!-- START CONTAINER -->
		<div class="content">

			<!-- START COLUMN1 - INTERACTIVE MENU -->
			<div id="menu_intmenu">
				<div id="btn_intmenu">
					<a href="#"><img src="<?php echo URL . 'public/img/btn_intmenu.png'; ?>"></a>	
				<!-- SLIDESHOW FOR INTERACTIVE MENU MENU -->	
				<div id="slideshow_intmenu"></div>	
				</div>	
			</div>	
			<!-- END INTERACTIVE MENU -->

			<!-- START COLUMN2 - GAMES-->
			<div id="menu_games">
				<div id="btn_games">
					<a href="#"><img src="<?php echo URL . 'public/img/btn_game.png'; ?>"></a>	
				<!-- SLIDESHOW FOR GAMES MENU -->	
				<div id="slideshow_games"></div>	
				</div>	
			</div>	
			<!-- END GAMES -->

			<!-- START COLUMN3 - VIDEOS -->
			<div id="menu_videos">
				<div id="btn_videos">
					<a href="#"><img src="<?php echo URL . 'public/img/btn_video.png'; ?>"></a>	
				<!-- SLIDESHOW FOR VIDEOS MENU -->	
				<div id="slideshow_videos"></div>	
				</div>	
			</div>	
			<!-- END VIDEOS-->
		</div>
		<!-- END CONTAINER -->
		</div>	
		<!-- END PAGE WRAPPER -->

